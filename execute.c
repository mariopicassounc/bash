...
static int
set_fd_in (scommand cmd)
{
    int redirected_in = 0;
    if (scommand_get_redir_in (cmd) != NULL)
        {
            int file_to_redirect_in
                = open (scommand_get_redir_in (cmd), O_DIRECTORY, O_RDONLY);
...
static void
execute_single_pipe (pipeline apipe)
{
    ...
    else
        {
            ...
            if (pid < 0)
                ...
                { /* Ejecuta el hijo */
                    ...
                    /* La memoria de este array la libera el SO cuando el hijo
                     * termina
                     */
...
static void
execute_external_cmd (scommand cmd)
{
    int status = set_fd_in (cmd);
...
/* Ejecuta dos comandos unidos por un pipe
 * identificando si son internos o no
 */
static void
execute_double_pipe (pipeline apipe)
{
    scommand cmd1 = pipeline_front (apipe); /* 1er comando */

    int fd[2]; /* file descriptor */
    pipe (fd); /* pipe que conecta la salida de cmd1 con la entrada del cmd2,
                  fd con extremos abiertos */

    int pid
        = fork (); /* Hijos heredan extremos de escritura y lectura abiertos */
    ...
    if (pid < 0)
        ...
        { /* Hijo 1 */
            ...
            /* Si es builtin se ejecuta dentro del hijo, comportamiento
             * observado en bash
             */
