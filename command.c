...
scommand_destroy (scommand self)
{
    ...
    /* This funtion pass as a argument the function to free all the elements of
     * the list */
...
{
    assert (self != NULL);
    return (self->cmd_args == NULL) && (self->redirect_in == NULL)
           && (self->redirect_out == NULL);
}
...
char *
scommand_front_and_pop (scommand self)
{
    assert (self != NULL && !scommand_is_empty (self));
...
char **
scommand_to_vector (scommand self)
{
    assert (!scommand_is_empty (self));
